  ...
  return (
    <div className='w-full min-h-screen bg-neutral-300'>
        ...
        <div className='p-4 ml-16'>
            <h1 className='text-sm mb-3 p-3 uppercase font-bold text-gray-700'>All Campaigns</h1>
            ...
            { 
                data.length > 0 && data.map( item => (
                    ...
                    <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                        <img src={`${process.env.REACT_APP_API_URL}/uploads/${item.cover}`} class="p-0 rounded-t-lg h-52"  alt="product image" />
                        

                        <div class="px-5 pb-5">
                            <div className='flex justify-between items-center'>
                                <h5 class="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white mt-2">{item.title}</h5>
                                <Link to={`/view_campaign/${item._id}`} className='mt-2'><VisibilityIcon /></Link>
                            </div>
